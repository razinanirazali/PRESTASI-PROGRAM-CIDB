...
QUARTER_CONFIG = {
    ...
    "Julai": {
        "code": "JUL",
        "title": "LAPORAN PRESTASI PROGRAM SEHINGGA JULAI 2026",
        "sheet_options": [
            "DATA DASHBOARD JULAI CLEAN",
            " DATA DASHBOARD JULAI CLEAN"
        ],
        # Sasaran kumulatif sehingga Julai: 7 daripada 12 bulan.
        "sasaran_panel": 58.33,
    },
}
...
# Program ini tidak dibajetkan dan tidak boleh dimasukkan dalam sebarang
# kiraan, senarai atau carta dashboard walaupun rekodnya masih wujud dalam
# mana-mana sheet Excel lama.
EXCLUDED_PROGRAM_PATTERNS = (
    "PEMBANGUNAN NATIONAL COMMON DATA ENVIRONMENT (CDE)",
)
...
st.markdown(
    ...
    .st-key-btn_tambahan_status button {
        background: transparent !important;
        border: none !important;
        box-shadow: none !important;
        color: #245be8 !important;
    ...
    .st-key-btn_tambahan_status button:hover {
    ...
    .st-key-btn_tambahan_status button p {
        color: #245be8 !important;
    ...
    .st-key-focus_btn_tambahan_status button {
        background: transparent !important;
        border: none !important;
        box-shadow: none !important;
        color: #245be8 !important;
    ...
    .st-key-focus_btn_tambahan_status button:hover {
        color: #1d4ed8 !important;
        text-decoration: underline !important;
        transform: none !important;
        border: none !important;
    ...
    .st-key-focus_btn_tambahan_status button p {
        color: #245be8 !important;
        font-size: 30px !important;
        font-weight: 900 !important;
        line-height: 1 !important;
...
def detect_status_khas(row):
    ...
    def ialah_permohonan_gugur(text):
        """Bezakan permohonan gugur daripada program yang telah digugurkan."""
        return bool(re.search(
            r"\b(?:MOHON|PERMOHONAN)\b.{0,120}\b(?:DI)?GUGUR(?:KAN)?\b",
            clean_upper(text),
        ))

    def ialah_pinda_program(text):
        """Program yang mempunyai pengganti ialah pindaan, bukan Gugur."""
        text = clean_upper(text)
        return (
            "PINDA PROGRAM" in text
            or "DIGANTIKAN DENGAN" in text
            or "GANTI DENGAN" in text
        )

    # Julai hanya merujuk CATATAN (JUSTIFIKASI) JULAI. Teks status lama
    # dalam kolum Q1/Q2 tidak boleh mempengaruhi klasifikasi semasa.
    if ACTIVE_QUARTER == "JUL":
        julai_text = clean_upper(row.get(Q2_JUSTIFIKASI_COL, ""))
        if ialah_pinda_program(julai_text):
            # Program lama yang diganti masih merupakan pergerakan keluar
            # bagi bulan Julai. Paparkan sebagai Gugur Julai selagi ia belum
            # pernah dikira Gugur dalam Q2. Program gantian direkodkan dan
            # dikira secara berasingan selepas diluluskan.
            if bool(row.get("GUGUR_Q2_TERDAHULU", False)):
                return "GUGUR Q2 (TERDAHULU)"
            return "TIDAK DILAKSANAKAN"
        gugur_diluluskan = (
            "GUGUR" in julai_text
            and not ialah_permohonan_gugur(julai_text)
        )
        if gugur_diluluskan or re.search(
            r"\bTIDAK\s+(AKAN\s+)?DILAKSANA(KAN)?\b", julai_text
        ):
            if bool(row.get("GUGUR_Q2_TERDAHULU", False)):
                return "GUGUR Q2 (TERDAHULU)"
            return "TIDAK DILAKSANAKAN"
        if re.search(r"\bBERMULA\s*Q3\b|\bMULA\s*Q3\b", julai_text):
            return "BERMULA Q3"
        if re.search(r"\bBERMULA\s*Q4\b|\bMULA\s*Q4\b", julai_text):
            return "BERMULA Q4"
        return ""

    # Sheet Q2 dan Julai mempunyai kolum khusus Bil. Program Gugur.
    if ACTIVE_QUARTER in ("Q2", "JUL"):
    ...
    if (
        any(pattern in combined_text for pattern in tidak_patterns)
        or (
            "GUGUR" in combined_text
            and not ialah_permohonan_gugur(combined_text)
        )
    ):
        return "TIDAK DILAKSANAKAN"
    ...
    # Q2/Julai: Q3 dan Q4 mesti dirujuk pada kolum justifikasi aktif sahaja.
...
def load_data(uploaded_file, sheet_options):
    ...
    # Index pandas masih mengekalkan kedudukan baris asal selepas baris kosong
    # dibuang; gunakan index itu untuk memadankan format sel Excel.
    df["_EXCEL_ROW"] = df.index + header_row + 2
    ...
    # Keluarkan program yang tidak dibajetkan sebelum sebarang pengiraan,
    # pengelasan status, filter, senarai dan bar chart dibentuk.
    description_col = next(
        (
            col for col in df.columns
            if clean_upper(col) == "DESKRIPSI PROGRAM"
        ),
        None,
    )
    if description_col is not None:
        description_upper = (
            df[description_col].fillna("").astype(str).map(clean_upper)
        )
        excluded_mask = pd.Series(False, index=df.index)
        for pattern in EXCLUDED_PROGRAM_PATTERNS:
            excluded_mask |= description_upper.str.contains(
                re.escape(pattern), regex=True, na=False
            )
        df = df.loc[~excluded_mask].copy()

    # Warna Traffic Light Julai telah ditetapkan dalam Excel. Baca warna
    # tersebut secara terus supaya formula kosong tetapi berwarna masih dikira.
    if "JULAI" in clean_upper(actual_sheet):
        workbook = load_workbook(uploaded_file, data_only=True, read_only=False)
        worksheet = workbook[actual_sheet]
        status_by_row = {}

        # Cari kolum % PENCAPAIAN (100%) Julai berdasarkan tajuk sebenar.
        # Jangan gunakan nombor kolum tetap kerana kedudukannya boleh berubah
        # apabila bahagian menambah atau membuang kolum dalam fail Excel.
        excel_header_row = header_row + 1
        julai_kpi_excel_col = None
        for column_number in range(1, worksheet.max_column + 1):
            header_value = clean_upper(
                worksheet.cell(
            ...
            if header_value in {
                ...
                # Ambil padanan terakhir kerana kolum Julai berada selepas Q2.
                julai_kpi_excel_col = column_number

        if julai_kpi_excel_col is None:
            workbook.close()
            raise ValueError(
                "Kolum % PENCAPAIAN (100%) Julai tidak dijumpai dalam Excel."
            )

        for excel_row in df["_EXCEL_ROW"].astype(int).tolist():
            cell = worksheet.cell(
                row=excel_row,
                column=julai_kpi_excel_col
            )
            colour = cell.fill.fgColor
            status = ""

            if cell.fill.fill_type == "solid":
                if colour.type == "theme" and colour.theme == 9:
                    status = "Hijau"
                elif colour.type == "rgb" and colour.rgb == "FFFFFF00":
                    status = "Kuning"
                elif colour.type == "rgb" and colour.rgb == "FFFF0000":
                    status = "Merah"

            status_by_row[excel_row] = status

        df["STATUS_WARNA_EXCEL"] = df["_EXCEL_ROW"].map(status_by_row).fillna("")
        workbook.close()

        # Senarai Gugur Julai ialah pertambahan baharu sahaja. Program yang
        # telah pun Gugur pada Q2 dikenal pasti melalui KOD + DESKRIPSI PROGRAM.
        q2_sheet = resolve_sheet_name(
            xls,
            QUARTER_CONFIG["Suku Kedua"]["sheet_options"]
        )
        df["GUGUR_Q2_TERDAHULU"] = False
        if q2_sheet is not None:
            q2_header = find_header_row(uploaded_file, q2_sheet)
            q2_df = pd.read_excel(
                uploaded_file,
                sheet_name=q2_sheet,
                header=q2_header
            )
            q2_text = q2_df.fillna("").astype(str).agg(
                " | ".join,
                axis=1
            ).str.upper()
            q2_gugur_mask = q2_text.str.contains(
                r"GUGUR|TIDAK (?:AKAN )?DILAKSANA",
                regex=True,
                na=False
            )

            # Rekod yang digantikan dengan program lain ialah PINDA PROGRAM,
            # bukan program Gugur.
            q2_pinda_mask = q2_text.str.contains(
                r"PINDA PROGRAM|DIGANTIKAN DENGAN|GANTI DENGAN",
                regex=True,
                na=False
            )
            q2_mohon_gugur_mask = q2_text.str.contains(
                r"(?:MOHON|PERMOHONAN).{0,120}(?:DI)?GUGUR(?:KAN)?",
                regex=True,
                na=False
            )
            q2_gugur_col = next(
                (
                    col for col in q2_df.columns
                    if clean_upper(col) in {
                        "BIL. PROGRAM GUGUR",
                        "BIL PROGRAM GUGUR"
                    }
                ),
            ...
            if q2_gugur_col is not None:
                q2_gugur_mask |= (
                    pd.to_numeric(q2_df[q2_gugur_col], errors="coerce")
                    .fillna(0)
                    .gt(0)
                )

            # Gunakan pengecualian selepas semua sumber penanda Gugur
            # digabungkan supaya nilai 1 dalam kolum Gugur tidak memasukkan
            # semula rekod Pinda Program atau permohonan yang belum diluluskan.
            q2_gugur_mask &= ~q2_pinda_mask & ~q2_mohon_gugur_mask

            def program_key(frame):
                return (
                    frame["KOD PROGRAM"].fillna("").astype(str).str.strip()
                    + "||"
                    + frame["DESKRIPSI PROGRAM"].fillna("").astype(str).str.strip()
                )

            q2_gugur_keys = set(program_key(q2_df[q2_gugur_mask]))
            df["GUGUR_Q2_TERDAHULU"] = program_key(df).isin(q2_gugur_keys)
...
def render_selected_list(df_list, sektor_col, bahagian_col, program_col, pencapaian_col, title):
    ...
    # Kumpulkan rekod mengikut Sektor dan Bahagian supaya bahagian yang sama
    # (contohnya BLA) tidak terpecah kepada beberapa kedudukan dalam jadual.
    sort_columns = [
        col for col in [sektor_col, bahagian_col, program_col]
        if col is not None and col in df_list.columns
    ]
    if sort_columns:
...
def get_chart_source_df(filtered_df):
    ...
    if selected == "Tambahan" and "PROGRAM_TAMBAHAN" in filtered_df.columns:
        return filtered_df[filtered_df["PROGRAM_TAMBAHAN"]].copy()
...
def build_main_dashboard_pdf(
    ...
    metrics = [
        ...
        ("SASARAN", f"{sasaran_panel:.1f}%"),
        ("PRESTASI", f"{prestasi_panel:.1f}%"),
        ("PENCAPAIAN", f"{pencapaian_panel:.1f}%"),
    ]
...
quarter_tab = st.radio(
    "Pilih Suku Tahun",
    options=["Suku Pertama", "Suku Kedua", "Julai"],
    index=2,
    horizontal=True,
...
if ACTIVE_QUARTER == "Q1":
    ...
    # Data kumulatif sehingga Julai.
    weightage_col = find_col_exact(df, [
        "SASARAN JULAI (KUMULATIF)",
        "SASARAN JULAI KUMULATIF"
    ])
    pencapaian_col = find_col_exact(df, [
        "PENCAPAIAN JULAI (KUMULATIF)",
        "PENCAPAIAN JULAI KUMULATIF"
    ])
    pencapaian_fizikal_col = find_col_exact(df, [
        "PENCAPAIAN JULAI",
        "PENCAPAIAN JULAI (KUMULATIF)",
        "PENCAPAIAN JULAI KUMULATIF"
    ])

    # Kolum pencapaian KPI Julai ialah kolum kedua yang bernama
    # % PENCAPAIAN (100%) dalam Excel (pandas menambah akhiran .1).
    q2_kpi_col = find_col_exact(df, [
        "% PENCAPAIAN (100%).1",
        "PENCAPAIAN (100%).1",
        "% PENCAPAIAN 100%.1"
...
if ACTIVE_QUARTER in ("Q2", "JUL") and q2_kpi_col is None:
    st.error(
        f"Kolum **% PENCAPAIAN (100%)** untuk {quarter_tab} "
        "tidak dijumpai dalam worksheet aktif."
    )
    st.write("Kolum yang dibaca:")
    st.write(list(df.columns))
    st.stop()

if ACTIVE_QUARTER in ("Q2", "JUL") and Q2_JUSTIFIKASI_COL is None:
    st.error(
...
# Program tambahan dikenal pasti daripada kolum rasmi Bil. Program DiTambah.
# Warna merah jambu pada lajur H ialah penanda visual dalam Excel sahaja.
tambahan_col = find_col_exact(df, [
    "BIL. PROGRAM DITAMBAH",
    "BIL PROGRAM DITAMBAH",
])
if tambahan_col is not None:
    df["PROGRAM_TAMBAHAN"] = (
        pd.to_numeric(df[tambahan_col], errors="coerce")
        .fillna(0)
        .gt(0)
    )
else:
    df["PROGRAM_TAMBAHAN"] = False
...
if ACTIVE_QUARTER == "JUL" and "STATUS_WARNA_EXCEL" in df.columns:
    # Untuk Julai, warna dikira terus daripada SEMUA rekod dalam sheet Julai.
    # Status Gugur/Q3/Q4 ialah penunjuk berasingan dan tidak mengurangkan
    # bilangan Hijau/Kuning/Merah yang memang telah diwarnakan dalam Excel.
    traffic_source_df = df.copy()

    if selected_sektor:
        traffic_source_df = traffic_source_df[
            traffic_source_df[sektor_col].isin(selected_sektor)
        ].copy()
    if selected_bahagian:
        traffic_source_df = traffic_source_df[
            traffic_source_df[bahagian_col].isin(selected_bahagian)
        ].copy()
    if selected_kod and kod_program_col is not None:
        traffic_source_df["KOD_PROGRAM_SHORT"] = (
            traffic_source_df[kod_program_col]
            .astype(str)
            .str.extract(r"PI\.[^.]+\.([^.]+)\.", expand=False)
            .fillna("LAIN-LAIN")
            .str.strip()
        ...
        traffic_source_df = traffic_source_df[
            traffic_source_df["KOD_PROGRAM_SHORT"].isin(selected_kod)
        ].copy()

    traffic_source_df = traffic_source_df[
        traffic_source_df["STATUS_WARNA_EXCEL"].isin(["Hijau", "Kuning", "Merah"])
    ].copy()
    df_hijau = traffic_source_df[
        traffic_source_df["STATUS_WARNA_EXCEL"] == "Hijau"
    ].copy()
    df_kuning = traffic_source_df[
        traffic_source_df["STATUS_WARNA_EXCEL"] == "Kuning"
    ].copy()
    df_merah = traffic_source_df[
        traffic_source_df["STATUS_WARNA_EXCEL"] == "Merah"
    ].copy()
    df_hijau["TRAFFIC_LIGHT"] = "🟢 Hijau"
    df_kuning["TRAFFIC_LIGHT"] = "🟡 Kuning"
    df_merah["TRAFFIC_LIGHT"] = "🔴 Merah"
else:
...
df_tambahan_filtered = filtered_df[filtered_df["PROGRAM_TAMBAHAN"]].copy()
...
tambahan = len(df_tambahan_filtered)

# =====================================================
# SENARAI PINDAAN PROGRAM / SASARAN (RUJUKAN SAHAJA)
# =====================================================
# Senarai ini dibina terus daripada Catatan (Justifikasi) Julai dan tidak
# digunakan dalam pengiraan Traffic Light atau Jumlah Program Aktif.
pindaan_source_df = df.copy()

if selected_sektor:
    pindaan_source_df = pindaan_source_df[
        pindaan_source_df[sektor_col].isin(selected_sektor)
    ].copy()

if selected_bahagian:
    pindaan_source_df = pindaan_source_df[
        pindaan_source_df[bahagian_col].isin(selected_bahagian)
    ].copy()

if selected_kod and kod_program_col is not None and kod_program_col in pindaan_source_df.columns:
    pindaan_source_df["KOD_PROGRAM_SHORT"] = (
        pindaan_source_df[kod_program_col]
        .astype(str)
        .str.extract(r"PI\.[^.]+\.([^.]+)\.", expand=False)
        .fillna("LAIN-LAIN")
        .str.strip()
    ...
    pindaan_source_df = pindaan_source_df[
        pindaan_source_df["KOD_PROGRAM_SHORT"].isin(selected_kod)
    ].copy()

if ACTIVE_QUARTER == "JUL" and JUSTIFIKASI_COL in pindaan_source_df.columns:
    pindaan_text = (
        pindaan_source_df[JUSTIFIKASI_COL]
        .fillna("")
        .astype(str)
    )

    mask_ada_gugur = pindaan_text.str.contains(
        r"\bGUGUR(?:KAN)?\b",
        case=False,
        regex=True,
        na=False,
    ...
    # Rekod Gugur tidak dipaparkan dalam Senarai Pindaan walaupun catatannya
    # menyebut program gantian. Senarai ini hanya untuk pindaan yang tidak
    # melibatkan status Gugur.
    mask_pinda_program = pindaan_text.str.contains(
        r"PINDAAN?\s+PROGRAM|PROGRAM.{0,50}(?:DIPINDA|DIUBAH|DIGANTI)|"
        r"(?:DIPINDA|DIUBAH|DIGANTI).{0,50}PROGRAM|"
        r"DIGANTIKAN DENGAN|GANTI DENGAN",
        case=False,
        regex=True,
        na=False,
    ) & ~mask_ada_gugur

    mask_pinda_sasaran = pindaan_text.str.contains(
        r"PINDA.{0,30}SASARAN|SASARAN.{0,30}PINDA|"
        r"SASARAN\s+BAHARU|"
        r"MOHON.{0,50}(?:PINDA|UBAH|KURANG).{0,50}(?:SASARAN|KPI)|"
        r"(?:SASARAN|KPI).{0,100}(?:DIPINDA|DIUBAH|DIKURANG|DITAMBAH|DITINGKAT)",
        case=False,
        regex=True,
        na=False,
    ) & ~mask_ada_gugur

    df_pinda_program = pindaan_source_df[mask_pinda_program].copy()
    df_pinda_sasaran = pindaan_source_df[mask_pinda_sasaran].copy()
else:
    df_pinda_program = pindaan_source_df.iloc[0:0].copy()
    df_pinda_sasaran = pindaan_source_df.iloc[0:0].copy()


def render_senarai_pindaan(key_suffix="main"):
    """Papar senarai pindaan tanpa mempengaruhi sebarang angka dashboard."""
    st.caption(
        "Senarai ini adalah untuk rujukan dan tidak mengubah Traffic Light "
        "atau Jumlah Program Aktif."
    )

    st.markdown(f"#### 🔄 PINDAAN PROGRAM ({len(df_pinda_program)})")
    if df_pinda_program.empty:
        st.info("Tiada pindaan program dikenal pasti berdasarkan filter semasa.")
    else:
        program_display = df_pinda_program[
            [sektor_col, bahagian_col, program_col, JUSTIFIKASI_COL]
        ].copy()
        program_display = program_display.rename(columns={
            sektor_col: "SEKTOR",
            bahagian_col: "BAHAGIAN",
            program_col: "PROGRAM ASAL",
            JUSTIFIKASI_COL: "PROGRAM GANTIAN / CATATAN PINDAAN",
        })
        st.dataframe(
            program_display,
            use_container_width=True,
            hide_index=True,
            height=min(520, 80 + (len(program_display) * 78)),
        )
        st.download_button(
            "⬇️ Download Senarai Pindaan Program",
            data=program_display.to_csv(index=False).encode("utf-8-sig"),
            file_name="senarai_pindaan_program_julai.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"download_pindaan_program_{key_suffix}",
        )

    st.markdown(f"#### 🎯 PINDAAN SASARAN / KPI ({len(df_pinda_sasaran)})")
    if df_pinda_sasaran.empty:
        st.info("Tiada pindaan sasaran/KPI dikenal pasti berdasarkan filter semasa.")
    else:
        sasaran_display = df_pinda_sasaran[
            [sektor_col, bahagian_col, program_col, JUSTIFIKASI_COL]
        ].copy()
        sasaran_display = sasaran_display.rename(columns={
            sektor_col: "SEKTOR",
            bahagian_col: "BAHAGIAN",
            program_col: "PROGRAM",
            JUSTIFIKASI_COL: "CATATAN PINDAAN SASARAN / KPI",
        })
        st.dataframe(
            sasaran_display,
            use_container_width=True,
            hide_index=True,
            height=min(620, 80 + (len(sasaran_display) * 78)),
        )
        st.download_button(
            "⬇️ Download Senarai Pindaan Sasaran / KPI",
            data=sasaran_display.to_csv(index=False).encode("utf-8-sig"),
            file_name="senarai_pindaan_sasaran_kpi_julai.csv",
            mime="text/csv",
            use_container_width=True,
            key=f"download_pindaan_sasaran_{key_suffix}",
        )
...
if ACTIVE_QUARTER == "JUL":
    # Jumlah Program Aktif tidak termasuk Gugur. Gunakan kategori yang sama
    # seperti paparan utama supaya angka panel sentiasa boleh direkonsiliasi:
    # Hijau + Kuning + Merah + Q3 + Q4.
    jumlah_program_panel = (
        hijau + kuning + merah + bermula_q3 + bermula_q4
    )
else:
    jumlah_program_panel = len(
        panel_count_df[
            ~panel_count_df["STATUS_KHAS"].isin([
                "TIDAK DILAKSANAKAN",
                "GUGUR Q2 (TERDAHULU)"
            ])
        ]
...
filtered_active_df = filtered_df[
    ~filtered_df["TRAFFIC_LIGHT"].eq("⚫ Gugur")
].copy()
...
# Sumber tunggal untuk carta dan summary Julai. Kategori warna menggunakan
# set rekod yang sama seperti bulatan Traffic Light dan senarai apabila diklik.
if ACTIVE_QUARTER == "JUL":
    status_frames = []
    for frame, category in [
        ...
        (df_kuning, "Kuning"),
        (df_merah, "Merah"),
        (df_bermula_q3_filtered, "Q3"),
        (df_bermula_q4_filtered, "Q4"),
        (df_tidak_filtered, "Gugur"),
    ]:
        if not frame.empty:
            frame_copy = frame.copy()
            frame_copy["KATEGORI_TRAFFIC"] = category
            status_frames.append(frame_copy)

    dashboard_status_df = (
        pd.concat(status_frames, ignore_index=True)
        if status_frames
        else filtered_df.iloc[0:0].copy()
    )
else:
    dashboard_status_df = filtered_df

if "selected_traffic" not in st.session_state:
    st.session_state.selected_traffic = "Semua"

chart_filtered_df = get_chart_source_df(dashboard_status_df)
    ...
    if st.session_state.focus_page == "chart":
        ...
        with focus_left:
            ...
            if ACTIVE_QUARTER == "Q1":
                ...
                if ACTIVE_QUARTER == "JUL":
                    fq3_label, fq3_value, fsep2, fq4_label, fq4_value, fsep3, fg_label, fg_value, fsep4, ft_label, ft_value = st.columns(
                        [0.48, 0.34, 0.12, 0.48, 0.34, 0.12, 0.82, 0.34, 0.12, 1.15, 0.34]
                    )
                else:
            ...
            if ACTIVE_QUARTER == "JUL":
                with fsep4:
                    st.markdown('<div class="compact-status-separator">|</div>', unsafe_allow_html=True)
                with ft_label:
                    st.markdown('<div class="compact-status-label">TAMBAHAN-</div>', unsafe_allow_html=True)
                with ft_value:
                    if st.button(str(tambahan), key="focus_btn_tambahan_status"):
                        st.session_state.selected_traffic = "Tambahan"
                        st.rerun()
        ...
        with focus_right:
            ...
            st.markdown(
                f"""
                    ...
                    <div class="summary-row">SASARAN <span>{sasaran_panel:.1f}%</span></div>
                    <div class="summary-row">PRESTASI <span>{prestasi_panel:.1f}%</span></div>
                    <div class="summary-line"></div>
                    <div class="summary-achievement">PENCAPAIAN {pencapaian_panel:.1f}%</div>
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        tab_senarai_focus, tab_summary_focus, tab_pindaan_focus = st.tabs([
        ...
        with tab_senarai_focus:
            ...
            if selected == "Semua":
                render_selected_list(filtered_active_df, sektor_col, bahagian_col, program_col, pencapaian_col, "Semua Program Aktif")
            elif selected == "Hijau":
            ...
            elif selected == "Tambahan":
                render_selected_list(df_tambahan_filtered, sektor_col, bahagian_col, program_col, pencapaian_col, "Program Tambahan")
            else:
                st.info("Klik Jumlah Program Aktif, bulatan Traffic Light, atau nilai Q2/Q3/Q4/Gugur/Tambahan untuk lihat senarai.")
        ...
        with tab_summary_focus:
            ...
            summary_df_focus = build_summary_program_bahagian(dashboard_status_df, bahagian_col)
        ...
        with tab_pindaan_focus:
            render_senarai_pindaan("focus")
...
with main_left:
    ...
    if ACTIVE_QUARTER == "Q1":
        ...
        if ACTIVE_QUARTER == "JUL":
            q3_label_col, q3_value_col, q_sep2, q4_label_col, q4_value_col, q_sep3, gugur_label_col, gugur_value_col, q_sep4, tambahan_label_col, tambahan_value_col = st.columns(
                [0.48, 0.34, 0.12, 0.48, 0.34, 0.12, 0.82, 0.34, 0.12, 1.15, 0.34]
            )
        else:
    ...
    if ACTIVE_QUARTER == "JUL":
        with q_sep4:
            st.markdown('<div class="compact-status-separator">|</div>', unsafe_allow_html=True)
        with tambahan_label_col:
            st.markdown('<div class="compact-status-label">TAMBAHAN-</div>', unsafe_allow_html=True)
        with tambahan_value_col:
            if st.button(str(tambahan), key="btn_tambahan_status"):
                st.session_state.selected_traffic = "Tambahan"
                if "selected_chart_bahagian" in st.session_state:
                    st.session_state.selected_chart_bahagian = None
                st.rerun()
...
tab_senarai, tab_summary, tab_pindaan = st.tabs([
    "📋 SENARAI STATUS PRESTASI",
    "📑 SUMMARY PROGRAM MENGIKUT BAHAGIAN",
    "📝 SENARAI PINDAAN"
])
...
with tab_senarai:
    ...
    if selected == "Semua":
        ...
        st.info(
            ...
            "Q2/Q3/Q4/Gugur/Tambahan untuk lihat senarai."
        )
