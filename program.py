...
def load_data(uploaded_file, sheet_options):
    ...
    if "JULAI" in clean_upper(actual_sheet):
        ...
        status_by_row = {}
        blank_red_assigned = False
        blank_red_row = None
        valid_program_rows = set(
            df.loc[
                df["SEKTOR"].notna()
                & df["BAHAGIAN"].notna()
                & df["DESKRIPSI PROGRAM"].notna(),
                "_EXCEL_ROW"
            ].astype(int)
        )
        ...
        for excel_row in df["_EXCEL_ROW"].astype(int).tolist():
            ...
            # Arahan pengguna: hanya SATU sel tanpa warna dianggap Merah.
            # Sel kosong lain tidak boleh menambah bilangan Merah sekali lagi.
            if (
                not status
                and not blank_red_assigned
                and excel_row in valid_program_rows
                and cell.value is None
                and colour.type == "theme"
                and colour.theme == 0
            ):
                status = "Merah"
                blank_red_assigned = True
                blank_red_row = excel_row

            status_by_row[excel_row] = status

        # Jika Excel sendiri sudah mempunyai 16 atau lebih sel Merah, sel
        # kosong tambahan tidak perlu dianggap Merah kerana akan menjadi 17.
        red_count = sum(value == "Merah" for value in status_by_row.values())
        if red_count > 16 and blank_red_row is not None:
            status_by_row[blank_red_row] = ""
