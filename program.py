...
EXCEL_CANDIDATES = [
    "Laporan Pencapaian Prestasi Fizikal Program CIDB Q1 2026 - DASHBOARD(3).xlsx",
    "Laporan Pencapaian Prestasi Fizikal Program CIDB Q1 2026 - DASHBOARD(2).xlsx",
    "Laporan Pencapaian Prestasi Fizikal Program CIDB Q1 2026 - DASHBOARD(1).xlsx",
    "Laporan Pencapaian Prestasi Fizikal Program CIDB Q1 2026 - DASHBOARD.xlsx",
]
EXCEL_PATH = next(
    (DATA_FOLDER / name for name in EXCEL_CANDIDATES if (DATA_FOLDER / name).exists()),
    DATA_FOLDER / EXCEL_CANDIDATES[0]
)
...
def detect_status_khas(row):
    ...
    if ACTIVE_QUARTER == "JUL":
        ...
        if "GUGUR" in julai_text or re.search(
...
def load_data(uploaded_file, sheet_options):
    ...
    if "JULAI" in clean_upper(actual_sheet):
        ...
        for excel_row in df["_EXCEL_ROW"].astype(int).tolist():
            ...
            # Arahan pengguna: satu sel tanpa warna dianggap Merah.
            if not status and cell.value is None and colour.type == "theme" and colour.theme == 0:
                status = "Merah"
...
def render_selected_list(df_list, sektor_col, bahagian_col, program_col, pencapaian_col, title):
    ...
    # Nombor ini mesti sama dengan nilai kategori yang diklik pada Traffic Light.
    st.caption(f"Jumlah program: {len(df_list):,}")
...
def get_chart_source_df(filtered_df):
    ...
    # Julai membaca Hijau/Kuning/Merah terus daripada warna Column AH.
    # Gunakan sumber yang sama untuk carta selepas bulatan diklik supaya
    # jumlah carta, bulatan dan senarai program sentiasa sepadan.
    if (
        ACTIVE_QUARTER == "JUL"
        and selected in {"Hijau", "Kuning", "Merah"}
        and "STATUS_WARNA_EXCEL" in filtered_df.columns
    ):
        colour_filtered = filtered_df[
            (filtered_df["STATUS_WARNA_EXCEL"] == selected)
            & (filtered_df["STATUS_KHAS"] != "GUGUR Q2 (TERDAHULU)")
        ].copy()

        # Permohonan untuk menggugurkan program belum bermaksud program telah
        # diluluskan sebagai Gugur. Apabila pengguna klik warna, kekalkan semua
        # rekod mengikut warna Excel pada carta dan jangan tukar kepada Gugur.
        icon_map = {
            "Hijau": "🟢 Hijau",
            "Kuning": "🟡 Kuning",
            "Merah": "🔴 Merah",
        }
        colour_filtered["KATEGORI_TRAFFIC"] = selected
        colour_filtered["TRAFFIC_LIGHT"] = icon_map[selected]
        return colour_filtered
...
df_gugur_q2_terdahulu = df[
    df["STATUS_KHAS"] == "GUGUR Q2 (TERDAHULU)"
].copy()
...
# Rekod Gugur Q2 terdahulu perlu kekal tersedia untuk bacaan warna Julai,
# tetapi tidak boleh ditambah kepada bilangan/senarai Gugur baharu Julai.
# Jika sel Julai mempunyai warna, gunakan warna itu untuk paparan senarai.
if ACTIVE_QUARTER == "JUL" and "STATUS_WARNA_EXCEL" in df_gugur_q2_terdahulu.columns:
    df_gugur_q2_terdahulu["TRAFFIC_LIGHT"] = (
        df_gugur_q2_terdahulu["STATUS_WARNA_EXCEL"]
        .map({
            "Hijau": "🟢 Hijau",
            "Kuning": "🟡 Kuning",
            "Merah": "🔴 Merah",
...
if ACTIVE_QUARTER == "JUL" and "STATUS_WARNA_EXCEL" in filtered_df.columns:
    # Untuk Julai, bilangan Hijau/Kuning/Merah dibaca daripada format warna
    # di Column AH Excel. Rekod Gugur Q2 terdahulu mesti dikecualikan daripada
    # warna supaya angka bulatan sama dengan senarai apabila ditapis.
    # Q3/Q4 dan 13 Gugur baharu kekal dikira melalui justifikasi Julai.
    traffic_source_df = filtered_df[
        filtered_df["STATUS_WARNA_EXCEL"].isin(["Hijau", "Kuning", "Merah"])
        & (filtered_df["STATUS_KHAS"] != "GUGUR Q2 (TERDAHULU)")
    ].copy()
